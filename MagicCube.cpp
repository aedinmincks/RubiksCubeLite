...
void CMagicCube::InitColors() {
  colors_.assign(6, std::vector<std::vector<char>>(
                        level_, std::vector<char>(level_, ' ')));
...
void CMagicCube::InitRtable() {
  ...
  for (int i = 0; i < level_; i++) {
    ...
    for (int j = 0; j < level_; j++) {
      v.emplace_back(
          &colors_[(int)EDirection::front][level_ - 1 - i][level_ - 1 - j]);
    }
  }
}
...
void CMagicCube::Rotate2DArray(std::vector<std::vector<char>> &arrs,
                               EAngle angle) {
...
bool CMagicCube::Rotate(EAxis axis, int start, int end, EAngle angle) {
  ...
  if (start == 0) {
    switch (axis) {
    case EAxis::x:
      Rotate2DArray(colors_[(int)EDirection::back],
                    (EAngle)((4 - (int)angle) % 4));
      break;
    case EAxis::y:
      Rotate2DArray(colors_[(int)EDirection::left],
                    (EAngle)((4 - (int)angle) % 4));
      break;
    case EAxis::z:
      Rotate2DArray(colors_[(int)EDirection::down],
                    (EAngle)((4 - (int)angle) % 4));
      break;
    default:
      break;
